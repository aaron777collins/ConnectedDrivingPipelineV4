...
class MConnectedDrivingLargeDataCleaner:
    def __init__(self, initialGathererPathprovider: IInitialGathererPathProvider, mlPathProvider: IMLPathProvider, contextprovider: IMLContextProvider):
        ...
        self.logger = Logger("MConnectedDrivingLargeDataCleaner")

        self.splitfilespath = self._initialgathererpathprovider.get("DataGatherer.splitfilespath")
        self.cleanedfilespath = self._mlpathprovider.get("MConnectedDrivingLargeDataCleaner.cleanedfilespath")
        self.combinedcleandatapath = self._mlpathprovider.get("MConnectedDrivingLargeDataCleaner.combinedcleandatapath")
        ...
        self.max_dist = self._contextprovider.get("MConnectedDrivingLargeDataCleaner.max_dist")

    def clean_data(self):

        filterFunction = self._contextprovider.get("MConnectedDrivingLargeDataCleaner.filterFunction")
        cleanFunc = self._contextprovider.get("MConnectedDrivingLargeDataCleaner.cleanFunc")
        ...
        for file in glob.glob(self.splitfilespath + "*.csv"):
            ...
            newDf = cleanFunc(dc).get_cleaned_data()
            if (filterFunction != None):
                newDf = filterFunction(newDf)
    ...
    def getNRows(self, n):
        ...
        dtypes = self._contextprovider.get("MConnectedDrivingLargeDataCleaner.dtypes", dtypes)
