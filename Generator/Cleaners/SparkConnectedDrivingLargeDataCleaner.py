"""
SparkConnectedDrivingLargeDataCleaner - PySpark implementation of ConnectedDrivingLargeDataCleaner.

This class provides PySpark-based large dataset cleaning for Connected Driving BSM datasets.
Instead of file splitting and combining (pandas approach), this uses Spark partitioning for
distributed processing of large datasets.

Key differences from pandas version:
- Uses Parquet partitioning instead of CSV file splitting
- No explicit file combining - Spark handles partitioned data transparently
- Uses ParquetCache instead of CSVCache
- DataFrame operations are distributed across cluster
...
class SparkConnectedDrivingLargeDataCleaner:
    """
    PySpark-based cleaner for large Connected Driving BSM datasets.

    This cleaner processes large datasets using Spark's distributed computing capabilities:
    1. Reads data using SparkDataGatherer (from Parquet or CSV)
    2. Cleans data using configurable cleaner class (e.g., SparkCleanWithTimestamps)
    ...
    def __init__(self, generatorPathProvider: IGeneratorPathProvider,
        ...
        """
        Initialize SparkConnectedDrivingLargeDataCleaner.
        ...
        self.logger = Logger("SparkConnectedDrivingLargeDataCleaner")

        # Get Spark session
        self.spark = SparkSessionManager.get_session()

        # Path configuration - NOTE: For Spark, these are Parquet directories, not individual files
        # splitfilespath: Input data directory (partitioned Parquet from SparkDataGatherer)
        ...
        # Cleaner class and functions (e.g., SparkCleanWithTimestamps, SparkCleanerWithFilterWithinRangeXY)
    ...
    def clean_data(self):
        """
        Execute data cleaning pipeline on entire dataset using Spark's distributed processing.
        ...
        2. Reads split data from SparkDataGatherer output (partitioned Parquet)
        ...
        self.logger.log("Starting large dataset cleaning with PySpark...")

        # Read split data (from DataGatherer output)
        # Note: SparkDataGatherer outputs partitioned Parquet, not individual CSV files
        if path.exists(self.splitfilespath) and self._is_valid_parquet_directory(self.splitfilespath):
            ...
            df = self.spark.read.parquet(self.splitfilespath)
        else:
            # If split files don't exist, use DataGatherer to create them
            self.logger.log("Split files not found. Using SparkDataGatherer to gather and split data...")
            gatherer = SparkDataGatherer()
            gatherer.gather_data()
            gatherer.split_large_data()
            df = self.spark.read.parquet(self.splitfilespath)

        self.logger.log(f"Loaded {df.count()} rows for cleaning")

        # Apply cleaner function to entire DataFrame
        # Note: cleanerClass is a Spark cleaner (e.g., SparkCleanWithTimestamps)
        ...
        cleaned_df.write.mode("overwrite").parquet(self.combinedcleandatapath)

        self.logger.log(f"Successfully cleaned {cleaned_df.count()} rows")
        return self

    def combine_data(self):
        """
        Combine cleaned data (no-op for Spark - data is already combined in Parquet directory).

        In the pandas version, this method combines multiple CSV files into one large file.
        For Spark, the partitioned Parquet directory already functions as a single logical dataset,
    ...
    def getNRows(self, n):
        ...
        Instead of pd.read_csv(nrows=n), this uses DataFrame.limit(n) for efficient sampling.
        ...
        Returns:
            DataFrame: PySpark DataFrame with N rows
        """
        ...
        df = self.spark.read.parquet(self.combinedcleandatapath)
        df_limited = df.limit(n)

        self.logger.log(f"Successfully retrieved {n} rows")
        return df_limited
    ...
    def getAllRows(self):
        ...
        Returns:
            DataFrame: PySpark DataFrame with all cleaned data
        """
        ...
        df = self.spark.read.parquet(self.combinedcleandatapath)
        self.logger.log(f"Successfully loaded {df.count()} rows")
        return df
