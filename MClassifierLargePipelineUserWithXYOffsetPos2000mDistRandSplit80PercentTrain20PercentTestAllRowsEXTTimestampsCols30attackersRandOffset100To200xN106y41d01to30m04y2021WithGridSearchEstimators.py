...
class MClassifierLargePipelineUserWithXYOffsetPos2000mDistRandSplit80PercentTrain20PercentTestAllRowsEXTTimestampsCols30attackersRandOffset100To200xN106y41d01to30m04y2021WithGridSearchEstimators:
    ...
    def run(self):
        ...
        for mclassifier, confusion_matrix in mcp.get_classifiers_and_confusion_matrices():
            classifier = mclassifier.classifier
            ...
            for key in keys:
                row = [" "]*len(CSV_COLUMNS)
                row[0] = key
                # The values is an array so we make each value have a cell
                for i in range(len(results[key])):
                    row[1+i] = results[key][i]
                self.csvWriter.addRow(row)
...
if __name__ == "__main__":
    mcplu = MClassifierLargePipelineUserWithXYOffsetPos2000mDistRandSplit80PercentTrain20PercentTestAllRowsEXTTimestampsCols30attackersRandOffset100To200xN106y41d01to30m04y2021WithGridSearchEstimators()
    mcplu.run()
