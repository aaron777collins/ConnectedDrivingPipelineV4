"""
Dask-based MClassifier Pipeline for RandOffset 50-100m attack detection.
...
class DaskMClassifierRandOffset50To100:
    """
    ...
    def run(self):
        self.logger.log("=" * 60)
        self.logger.log("DASK PIPELINE: RandOffset 50-100m Attack Detection")
        self.logger.log("=" * 60)
        ...
        self.logger.log("Adding RandOffset 50-100m attacks to train set...")
        train = StandardPositionFromOriginAttacker(train, "train").add_attackers().add_attacks_positional_offset_rand(min_dist=50, max_dist=100).get_data()
        
        self.logger.log("Adding RandOffset 50-100m attacks to test set...")
        test = StandardPositionFromOriginAttacker(test, "test").add_attackers().add_attacks_positional_offset_rand(min_dist=50, max_dist=100).get_data()
...
if __name__ == "__main__":
    mcplu = DaskMClassifierRandOffset50To100()
    mcplu.run()
